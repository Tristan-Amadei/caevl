...
class AutoEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        canny_edges : bool, optional
            ...
            In that case, the last layer of the decoder is activated with a sigmoid, by default False.
        device : str, optional
        ...
        flag_perceptual_loss : bool, optional
            If True, perceptual loss is added to the training loss, by default False.
        scalar_divide_channels_fst_feature_map_for_decoder : int, optional
            The first feature map of the decoder 
            has nb_channels_fst_feature_map // scalar_divide_channels_fst_feature_map_for_decoder channels, by default 1.
        ...
        self.perceptual_loss = Perceptual_DinoV2(device=self.device, grayscale_shift=grayscale_shift, 
                                                 use_cbam=use_cbam, cbam_channels=[self.nb_input_channels, 6, 3]) if flag_perceptual_loss else None
        self.alpha = alpha
        ...
        # initialize encoder
        self.encoder = Encoder(self.nb_input_channels, self.nb_channels_fst_feature_map, self.latent_dim)
        ...
        if self.encoder_only:
            ...
            self.decoder = Decoder(self.nb_input_channels, nb_channels_fst_feature_map_decoder, self.latent_dim, 
                                self.last_feature_size, canny_edges, softmax, sigmoid)
