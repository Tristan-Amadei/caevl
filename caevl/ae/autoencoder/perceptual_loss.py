...
class Perceptual_DinoV2(nn.Module):
    ...
    grayscale_shift : bool
        ...
        Grayscale shift takes a grayscale image and repeats it on 3 channels,
        but each channel is shifted by one pixel.
    ...
    def __init__(self, device, grayscale_shift,
        ...
        self.dino = DinoV2(device,
                           load_pretrained_weights=True,
                           requires_grad=False,
                           add_head=False,
                           grayscale_shift=grayscale_shift,
                           no_grayscale=use_cbam)
        self.dino.eval()

        self.use_cbam = use_cbam
        if self.use_cbam:
            self.cbam_bottleneck = CBAM_Bottleneck(channels=cbam_channels,
                                                   kernel_sizes=None,
                                                   stride=1,
                                                   bias=False,
